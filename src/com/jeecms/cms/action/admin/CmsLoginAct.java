...
public class CmsLoginAct {
    ...
    /**
     * 简化ibscms后台访问路径
     * 映射/admin访问路径，在AdminContextInterceptor.java中getURI方法做了特殊过滤跳转 <br/>
     * 2013.3.17 yangq
     */
    ...
    private WebErrors validateSubmit(String username,
                                     String password,
                                     String captcha,
                                     Integer errorRemaining,
                                     HttpServletRequest request,
                                     HttpServletResponse response) {
        ...
        if (!StringUtils.isBlank(captcha) || (errorRemaining != null && errorRemaining < 0)) {
            ...
            try {
                if (!imageCaptchaService.validateResponseForID(session.getSessionId(request,
                                                                                    response),
                                                               captcha)) {
