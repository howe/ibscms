...
public class CmsUser extends BaseCmsUser implements PriorityInterface {
	...
	public Set<Channel> getChannelsByDepartment(Integer siteId) {
		Set<Channel> set;
		Set<Channel> results = new HashSet<Channel>();
		Set<CmsDepartment>departs=getDepartments();
		for(CmsDepartment d:departs){
			set=d.getChannels();
			for (Channel c : set) {
				if (c.getSite().getId().equals(siteId)) {
					results.add(c);
	...
	public Integer[] getDepartmentIds() {
		Set<CmsDepartment> departments = getDepartments();
		return CmsDepartment.fetchIds(departments);
	}
	...
	public void addToDepartments(CmsDepartment department) {
		if (department == null) {
			return;
		}
		Set<CmsDepartment> set = getDepartments();
		if (set == null) {
			set = new HashSet<CmsDepartment>();
			setDepartments(set);
		}
		set.add(department);
	}
